...
/**
 * Anthropic Provider Crawler
 * Fetches current pricing from Anthropic Claude models
 * Falls back to manual data if API fetch fails
 *
 * Current Anthropic pricing (as of 2026-02):
 ...
 * Note: Anthropic doesn't have a public pricing API endpoint
 * This returns static fallback data that can be updated periodically
 */
export async function fetchAnthropicPricing() {
  try {
    // Anthropic doesn't expose a pricing API, so we use verified static data
    // In production, this could scrape their pricing page or use an API when available
    logger.info('anthropic_pricing_fetch', { source: 'verified_static_data', date: '2026-02' });
    return FALLBACK_MODELS;
  } catch (error) {
    logger.error('anthropic_pricing_fetch_failed', { error: error.message });
    return [];
  }
}

/**
 * Seed Anthropic pricing data into database
 * Creates provider entry if doesn't exist, then upserts services and rates
 */
export async function seedAnthropic() {
  const db = getDb();
  ...
  try {
    ...
    if (models.length === 0) {
      logger.warn('anthropic_seed_no_data', {});
    ...
    `).run('Anthropic', 'https://anthropic.com', 'llm');

    const providerId = db.prepare(`SELECT id FROM providers WHERE name = ?`).get('Anthropic').id;
    ...
    logger.info('anthropic_seed_complete', { models: models.length, rates: ratesAdded });
    return { providers: 1, services: models.length, rates: ratesAdded };

  } catch (error) {
    logger.error('anthropic_seed_failed', { error: error.message });
    return { providers: 0, services: 0, rates: 0 };
  }
}

/**
 * Crawl Anthropic pricing for crawler orchestrator
 * Returns standardized rate objects for insertion
 */
export async function crawlAnthropic() {
  try {
    const models = await fetchAnthropicPricing();
    ...
    logger.error('anthropic_crawl_failed', { error: error.message });
    return [];
  }
}

export default { fetchAnthropicPricing, seedAnthropic, crawlAnthropic };
